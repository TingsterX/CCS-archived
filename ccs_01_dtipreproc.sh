...
if [ $# -lt 8 ];
then
        echo -e "\033[47;35m Usage: $0 subject analysis_dir dti_name dti_dir_name dir_rep b0_rep num_seeds anat_dir_name \033[0m "
        exit
fi
...
then
	...
	more ${dti}.bval | cut -d ' ' -f1-${N} > bval_cut
	more ${dti}.bvec | cut -d ' ' -f1-${N} > bvec_cut
	...
	mkdir -p dtk ; cd dtk
	rm -v gradient.txt ; 1dtranspose ${dti_dir}/bvec_cut gradient.txt ; 
	Nb0=$(echo "scale=0; ${b0_rep}+1"|bc) ; 
	bvalue=`more ${dti_dir}/${dti}.bval | cut -d ' ' -f${Nb0}-${Nb0}`; 
	echo "The B-value is ${bvalue} for non-b0 images." #need test!
	dti_recon ${dti_dir}/data.nii.gz dtifit -gm gradient.txt -b ${bvalue} -b0 ${b0_rep} -it nii.gz -ot nii.gz
fi

## 6. DTK fiber tracking (some parameters need to be decided finally)
echo "DTI fiber tracking (DTK)"
mkdir -p ${dti_dir}/track ; cd ${dti_dir}/track
echo "Tracking fibers with ${num_seeds} random seeds in each voxel ..."
dti_tracker ${dti_dir}/dtk/dtifit tracks_${num_seeds}seeds.trk -it nii.gz -at 45 -rseed ${num_seeds} -m ${dti_dir}/b0_brain_mask.nii.gz -m2 ${dti_dir}/dtk/dtifit_fa.nii.gz 0.1
spline_filter tracks_${num_seeds}seeds.trk 0.5 tracks_${num_seeds}seeds_spline.trk
