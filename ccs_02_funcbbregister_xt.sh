...
#		highres=${dir}/${subject}/${anat_dir_name}/reg/highres.nii.gz
#		flirt -ref ${highres} -in ${mov} -omat flirt_example_func2highres.mat -cost corratio -dof 6 -interp trilinear
#		flirt -ref ${highres} -in ${mov} -applyxfm -init flirt_example_func2highres.mat -noresample -out flirt_example_func2highres.nii.gz
		3dresample -orient RSP -prefix ${func_dir}/reg/tmp_example_func_brain_rsp.nii.gz -inset ${mov}
                fslreorient2std ${func_dir}/reg/tmp_example_func_brain_rsp.nii.gz > ${func_dir}/reg/rsp2rpi.mat
                convert_xfm -omat ${func_dir}/reg/rpi2rsp.mat -inverse ${func_dir}/reg/rsp2rpi.mat
                mov=${func_dir}/reg/tmp_example_func_brain_rsp.nii.gz
...
#	if [ ! -e ${func_mask_dir}/brain.mni305.2mm.nii.gz ]; then
		...
		rm brain.nii.gz
		3dresample -orient RSP -prefix ${func_mask_dir}/brain.nii.gz -inset ${func_dir}/${rest}_pp_mask.nii.gz
		for hemi in lh rh
		do
			#surf-mask
