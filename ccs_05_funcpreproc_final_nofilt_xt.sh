...
then
        ...
        then
                for hemi in lh rh
                do
			rm -f ${FC_dir}/tmp_${rest}_pp_nofilt_sm0.nii.gz
			3dresample -orient RSP -prefix ${FC_dir}/tmp_${rest}_pp_nofilt_sm0.nii.gz -inset ${FC_dir}/${rest}_pp_nofilt_sm0.nii.gz
                        mri_vol2surf --mov ${FC_dir}/tmp_${rest}_pp_nofilt_sm0.nii.gz --reg ${func_reg_dir}/bbregister.dof6.dat --trgsubject ${fsaverage} --interp trilin --projfrac 0.5 --hemi ${hemi} --o ${FC_dir}/${rest}.pp.nofilt.sm0.${fsaverage}.${hemi}.nii.gz --noreshape --cortex --surfreg sphere.reg
			rm ${FC_dir}/tmp_${rest}_pp_nofilt_sm0.nii.gz
